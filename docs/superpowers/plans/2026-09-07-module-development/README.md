...
База планирования: 652ba859efe0cf186236fd245a5aaa348e27958f, module 1.4.245. R0 реализован в 1.4.246; результаты проверок и ограничение серверной metadata записаны в его плане. R1 реализован в 1.4.247; результаты live QA и ограничения загрузки записаны в его плане. R2 реализован в 1.4.248; для штатного socket route требуется обновить сессию active GM. R3 реализован в 1.4.249; native двухклиентные GM socket проверки пройдены после обновления сессии, отдельный player и complex whole transfer остаются ограничениями. R4 реализован в 1.4.250; см. профильный отчёт. R5 реализован в 1.4.251 на странице «Группа» инвентаря; native GM QA пройден, отдельная player-сессия остаётся ограничением. R6 реализован в 1.4.252, сквозная live-приёмка ждёт обновления active GM; см. отчёт R6. R7: 35 профилей реализованы в 1.4.255 (пассивные, оружейные и поглощение); 2 кандидата ожидают уточнения правил, часть multiplayer QA открыта. R8 частично реализован в 1.4.263: GM-only durable prepare-result/private Chat draft, общий catalog/price reader, catalog gate перед новым Item ingress и prepared graph recovery. Незавершённые claims резервируют строки/монеты; typed выдача персонажу использует полный graph и тот же source claim. Публикация и безопасный Chat drag подключены; настройки окна, preview, direct grant и reopen реализованы, полная live-приёмка ещё открыта. R9 частично реализован в 1.4.266: canonical metadata/value, capacity и pure recursive generation с общим ledger. Выдача/материализация/UI ещё открыты. Для отсутствующего объёма явно принято рабочее допущение weight/count + проверка известных volumes. R10 ещё не реализован, R11 отложен. Пользователь явно попросил подробные планы всех этапов; работу продолжаем в текущей задаче без смены модели.
...
| R5 | [Слава и дурная слава](r5-reputation.md) | Инвентарь → Группа; уточнение 2026-09-08 |
...
- Слава/дурная слава — два счётчика в инвентаре на странице «Группа» (уточнение 2026-09-08); варианты Космологии и XP отменены.
