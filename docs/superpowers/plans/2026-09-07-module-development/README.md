...
Актуальная реализация: 1.4.280. R0/R1/R2/R4 закрыты по профильным планам. R3/R5 реализованы, отдельная player/fault приёмка открыта. R6 реализован, сквозная проверка ждёт доступного обновлённого GM и разрешения на QA Chat. R7:35 implemented,2 спорных candidate,11 прежних curses,43 unavailable; остаток live/coverage открыт. R8/R9 реализуют генерацию, полную стоимость, выдачу и перенос составных деревьев; полная Chat/player/fault цепочка не закрыта. R10 реализован: окно и список действий, без изменения ресурсов/времени; остаток multiplayer/zoom приёмки открыт. R11 отложен и не входит в реализацию. Подтверждения и точные ограничения находятся в планах этапов; зелёные unit tests не заменяют открытые live gates.
