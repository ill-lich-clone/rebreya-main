...
- [ ] Написать геометрический тест с реальными граничными числами:
```js
...
```
- [ ] Запустить node --test tests/anchored-overlay.test.mjs, получить red на missing module.
- [ ] Реализовать pure расчёт:
```js
...
- [ ] Реализовать AnchoredOverlay: body portal с position:fixed; content передаётся DOM node; text через textContent. show заменяет content одного portal, onClose ровно один раз на открытие. reposition читает свежий anchor, measured size и viewport, вычисляет style и слой выше computed z-index окон.
- [ ] Подписать capture scroll, resize, ResizeObserver owner/overlay и движение ApplicationV2. Все listeners принадлежат AbortController; close отключает временные подписки и возвращает focus, destroy также освобождает references.
- [ ] Добавить fake-DOM lifecycle tests: show×3 = один portal; anchor detached → close; destroy×2 безопасен; observers disconnected; Escape не закрывает другое окно. Не использовать приватный ApplicationV2._maxZ.
...
- [ ] В existing createApp test fixture добавить наблюдение portal; воспроизвести открытие нижней строки и rerender selection.
- [ ] Заменить вычисление положения от rowIndex*80 CSS px вызовом helper. Data-action обработчики продолжают принадлежать StorageApp: передать обработку portal events тому же action dispatcher, чтобы body portal не потерял bubbling в window root.
- [ ] Anchor разрешается каждый раз по data-anchor-row-id и точной item row; старый HTMLElement после rerender не переиспользуется.
- [ ] _onClose/detached row/breadcrumb change закрывают overlay. Не переносить source row mutation логику в helper; transfer pending/error refresh остаются прежними.
- [ ] Из CSS удалить позиционные anchorRow/anchorColumn формулы для этого popover, сохранить scoped theme, кнопки и внутренний overflow:auto. Проверить coin/journal/обычный Item и вложенное хранилище.
- [ ] Тесты подтверждают, что "Взять"/quantity/broken actions из portal вызывают прежние API ровно один раз и закрытие не удаляет storage row.
...
- [ ] Добавить тест prepared slot с длинным itemName и HTML-подобным именем: aria-label/textContent безопасны; tooltip не становится raw HTML; native title на slot target отсутствует.
- [ ] В HBS убрать itemName/itemMeta внутри fixed square. Оставить img и необходимые state badges; label анатомического слота вне квадрата; имена списка справа остаются.
- [ ] Ядро scoped CSS:
```css
...
- [ ] Добавить styled tooltip через AnchoredOverlay на hover/focus, Escape/blur/leave закрывают. Clear/open/drag actions сохраняют existing stable data attributes.
- [ ] Проверить bindHeroDoll lifecycle через existing WeakMap AbortControllers: rerender отменяет старые handlers, одно нажатие вызывает одно действие.
- [ ] В UI test показать два разных source IDs с одним именем; R1 не делит stack и не исправляет identity из R3.
