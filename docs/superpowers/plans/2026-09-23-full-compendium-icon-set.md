...
> **For agentic workers:** REQUIRED SUB-SKILL: use `superpowers:executing-plans` before code changes. Для Luna Max: выполнять задачи последовательно, отмечая `- [x]` только после записанного доказательства. Одновременно генерировать только один `packId`. Новый чат брать на следующий pack с коротким handoff и receipts предыдущего; не поручать отдельные мелкие шаги субагентам.
...
## Переход между чатами Luna Max

Следующий чат получает: ссылку на эту спецификацию и план, текущий commit/version, один `packId`, styleKey, SHA manifest, путь backup/restore receipt, последний установленный batch ID, список нерешённых коллизий, already-run checks и точный следующий шаг. Не копировать длинные логи или весь manifest в prompt. При новом pack снова выполнить Git preflight и проверить backup receipt, но не повторять baseline sync и генерацию завершённых batches без причины.
