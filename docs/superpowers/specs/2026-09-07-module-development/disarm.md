...
## Решения Q2 — уточнение пользователя 2026-09-08

Базовая формула: d20 + постоянный ability modifier выбранного оружия + его действительный proficiency contribution. Quality/magical attack bonus, Bless/Bardic/прочие временные добавки исключены. Уточнение пользователя: «Только характеристика и владение, без бонусов черт». Все class/feat attack modifiers исключаются; GM не добавляет их обратно через baseline. Изменения ability score временным эффектом тоже требуют baseline, а не механического чтения уже buffed modifier.

Рабочее допущение: обычная дистанция выбранного weapon mode, включая ranged, с range/visibility ограничениями. Пользователь уточнил выбор точки и состав формулы, отдельного ограничения ближним боем не добавил.
...
Уточнение пользователя: «Выбирать случайную точку так, чтобы за стену не падало. После выпадения убирать предмет из руки персонажа». Перед случайным выбором исключаются соседние точки за стенами и границами сцены. При восьми доступных направлениях — к8, при меньшем числе — равномерный выбор среди доступных точек с отображением числа вариантов. Hex использует соседние клетки, gridless — восемь соседних направлений. Если доступных точек нет, source не списывается, операция требует сверки. После подтверждённого переноса очищаются руки/equipped/ячейки куклы; это часть recoverable transfer.
