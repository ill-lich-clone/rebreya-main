...
- `Боеприпасы`, `Особые боеприпа` и/или актуальный metadata-resolved `Особые боеприпасы` — обычные и специальные боеприпасы; registry явно задаёт допустимый active source и отвергает неоднозначность двух непустых источников;
