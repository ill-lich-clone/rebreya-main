...
- `Боеприпасы` и metadata-resolved `Особые боеприпасы` — обычные и специальные боеприпасы. `Боеприпасы` читается как raw multi-table sheet: основной блок `B3:F17` и блок боеприпасов ручного гранатомёта/ручницы `B19:G...`; пустой заголовок `D19` трактуется как `Ранг`, а глобальное правило `G19` принадлежит всему второму блоку и означает уменьшение каждой кости урона ручницы на одну категорию. Скрытый `Особые боеприпа` является подтверждённым legacy-зеркалом со смещением диапазона: importer принимает его только пока нормализованные матрицы полностью эквивалентны каноническому листу; любое расхождение блокирует импорт как неоднозначный источник. Пока `_СПРАВОЧНИК_СНАРЯЖЕНИЯ` хранит специальные боеприпасы в прежней горизонтальной системе координат, exact join канонической строки `Особые боеприпасы!B<row>` разрешает только объявленный alias `Боеприпасы!H<row-1>`; name fallback запрещён;
