...
- `getUpgradeAvailability(productId, manifest=[])` → `{decision,available,label,reason}`. Доступны только existing-curse/simple-implemented; неизвестный ID → unavailable-no-rule. Все 124 строки имеют явное решение: 11 проклятий, 35 реализованных простых, 2 кандидата, 41 сложное недоступное и 35 без проверенного правила. Новые строки полного синка добавляются fail-closed как unavailable-no-rule до отдельного аудита автоматизации. Planned-R7 tests не являются свидетельством реализации.
...
- Focused: `tests/upgrade-automation-manifest.test.mjs`; историческая матрица в `docs/superpowers/specs/2026-09-07-module-development/upgrade-scope.md` описывает исходные 91 решение, а текущим полным источником для 124 строк служит JSON manifest.
...
- `getItemUpgradeCategory(item)` допускает только weapon; equipment light/medium/heavy/shield/clothing как outerwear; native wondrous/staff/rod/wand либо canonical `flags.rebreya-main.itemType` Чудесный предмет/Посох/Жезл/Волшебная палочка как wondrous. Обычные loot, treasure, consumable, ring, tool и container без такого явного source type не являются host. Каталог снаряжения заранее проецирует распознанную носимую одежду в native `equipment/clothing`; legacy Actor-owned `loot/gear` признаётся одеждой только по exact сочетанию `equipmentType:Снаряжение` и stable `gearId` из `isRebreyaWearableClothingGearId(gearId)`. Сам resolver имена не использует. `buildUpgradeHostDescriptor(item)` адаптирует разрешённую категорию в canonical tags; явные редкие qualifiers берёт из `flags.rebreya-main.upgradeCompatibilityTags`. Состояние рук/экипировки читает через `getItemHeldHands`/`isItemEquipped`, attuned — native boolean, broken — canonical durability state.
...
- `createItemUpgradePanelHtml(hostItem)`/`renderItemUpgradePanel(root,hostItem)` показывают обычную панель только допустимому host, но сохраняют removal-only панель для исторически запрещённого host с установленными links: без capacity и drop controls. `bindItemUpgradeSheet(root,app,moduleApi)` подключает install/drop только допустимой цели, а open/remove — также cleanup-панели; `removeItemUpgrade` остаётся canonical способом безопасно снять historical child.
...
- R9: shared traversal считает canonical snapshot через reader.readContainerValueNodes; root depth0/max8, cap200 documents, unique host/upgrade/container identities, quantity1 для контейнера. contentsValue включает только оставшиеся children/currency. Без адаптера сохраняется unsupported-container. Подробный data flow/errors и тесты — [паспорт составного лута](lootgen-composite-function-passport.md). Расчёт не меняет источники и не пересчитывает world Items.
