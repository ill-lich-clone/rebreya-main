...
| `long-project` | Частично, но сам V2 делегирует детали внешним правилам. | `rankChoice` задаёт ранг 1-9; `projectCounter` даёт 4/6/8 делений для рангов 1-3/4-6/7-9; `resources` хранит вложенные зм за неделю; `check` настраивается мастером; `downtimeResult` считает сдвиг счётчика от превышения DC. | Цель проекта, риски, итоговое нарративное подтверждение и фактическое списание золота остаются за мастером. |
...
## 2026-06-10: Вычисляемые итоги конструктора

- Добавлен общий расчёт `downtimeResult.resultFormula`: итоговое действие может брать поля предыдущих целевых действий (`success`, `total`, `quantity`, `computedTotal`, `formulaResult`, `selectedValue`), применять `operator` и `multiplier`, получать одно число и прогонять его через собственные пороги.
- `createRequest` теперь сразу пересчитывает итоговые действия, если они зависят только от выбранных игроком ресурсов/чисел. `recordCheckResult` пересчитывает итог после каждого записанного броска.
- Пересобраны данные:
  - `craft` и `firearm-crafting` считают прогресс от рабочих дней;
  - `firearm-development`, `gambling`, `fighting-tournament`, `profession-work` и `long-project` получают итог из предыдущих проверок;
  - `laboratory-alchemy` получила выбор уровня зелья и стоимость через rank-aware resources;
  - `construct-crafting` хранит вычисляемый прогресс от выбранных ресурсов;
  - `research` остаётся на схеме `check thresholds -> resultMapping -> fragments`.
- UI конструктора теперь различает два вида `downtimeResult`: маппинг порога одной проверки и формульный итог с порогами. Для формульного итога показываются source/field/operator/multiplier и сохраняются подписи порогов.
- Остаточный долг: `formulaRoll` на вкладке игрока пока только отображается, поэтому `magic-item-purchase` структурирован, но полноценный ввод/бросок цены ещё требует отдельной доработки.

## 2026-06-12: Длительные проекты

- `long-project` переведён на общий конструктор: выбор ранга, счётчик проекта, недельные вложения, настраиваемая проверка и вычисляемый сдвиг счётчика.
- Деления счётчика фиксированы по правилам: ранги `1-3` = 4 деления, `4-6` = 6 делений, `7-9` = 8 делений.
- Карточка заявки показывает недельный результат и визуальный счётчик; при наличии свободной недели можно отправить новую заявку “Продолжить работу” с сохранённым прогрессом и настройками проверки.
