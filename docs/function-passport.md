...
- **Тесты:** `tests/ui-refresh-coordinator.test.mjs` (включая scoped city selection), `tests/main-composition-root.test.mjs`, `background-refresh-focus.test.mjs`, `style-theme.test.mjs` и профильные `*-app-context.test.mjs`/`*-ui.test.mjs`.
