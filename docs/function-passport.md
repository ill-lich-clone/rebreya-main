...
- **Тесты:** `tests/economy-city-connections.test.mjs`, `tests/city-presentation-overrides.test.mjs`, `tests/public-economy-read-model.test.mjs`, профильные tests сервисов/окон и `tests/main-composition-root.test.mjs` для wiring.
...
- **Внутренние refresh entrypoints:** `refreshInventoryViews({ actorIds })`, `refreshDowntimeViews({ actorIds })`, `refreshCosmologyViews()`, `refreshOpenApps()`. Они вызываются после authoritative mutation result и не являются доменными командами.
...
- **Тесты:** `tests/ui-refresh-coordinator.test.mjs`, `background-refresh-focus.test.mjs`, `style-theme.test.mjs` и профильные `*-app-context.test.mjs`/`*-ui.test.mjs`.
