...
- **Тесты:** весь кластер `tests/storage-*.test.mjs`, особенно `storage-service`, `storage-access`, `storage-socket`, `storage-container-hierarchy`, `storage-deposit-source`; каталог и lifecycle встроенных Actor — `tests/builtin-storage-presets.test.mjs`, `tests/builtin-storage-actor-service.test.mjs`.
