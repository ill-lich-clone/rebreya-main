...
- **Космология:** расчёты Mechanus — `scripts/cosmology/mechanus-rolls.js`; явный нулевой dice count сохраняется как `0`, а advantage/disadvantage d20 всегда заменяет системный keep-результат первым брошенным кубом и плоским модификатором `+2`/`-2`; внешние методы `getCosmologyState()`, `isMechanusEnabled()`, `setMechanusEnabled(enabled)`, `openCosmologyApp()`; typed command `cosmology.setMechanus`; UI — `scripts/ui/cosmology-app.js`.
