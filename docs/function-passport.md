...
- **Journal read contract:** `StorageJournalReader.read(journalUuid)` live-resolves only `JournalEntry`, sorts its pages by numeric `sort` then ID, enriches text through Foundry 13 `CONFIG.ux.TextEditor.implementation.enrichHTML` with `secrets:false` and documents/links/embeds/rolls/custom disabled, rejects any remaining unrevealed `section.secret`, and returns only plain `{ name, pages }` data with whitelisted page fields plus derived `html`. Resolution/enrichment/deletion failures collapse to one unavailable error; Journal/page UUIDs, ownership, flags, sheet/embedded document data and raw `text.content` never enter the snapshot, and the reader never updates the Journal.
- **Journal read data flow/security:** public `readStorageJournal(tokenUuid, rowId, request)` builds only `{ tokenUuid, characterTokenUuid, rowId, path? }`; exact `isValidStorageJournalReadPayload()` guards `storage.journal.read`. On every request `StorageCommandService.readJournal()` reuses `#resolveAccess`, requires opened state, live-reads `readStorageStateAtPath()`, requires an unclaimed `isStorageJournalRow()` and passes only authoritative `row.sourceId` to the reader. Non-GM `getStorageSnapshot()` removes Journal `sourceId`; Journal sidebar visibility/ownership is neither inspected nor changed. Focused coverage: `tests/storage-journal-reader.test.mjs`, `tests/storage-socket.test.mjs`, `tests/storage-main-registration.test.mjs`, `tests/security.test.mjs`.
