...
- **Тесты:** `tests/economy-city-connections.test.mjs`, `tests/city-presentation-overrides.test.mjs`, `tests/public-economy-read-model.test.mjs`, профильные tests сервисов/окон, `tests/main-composition-root.test.mjs` для wiring/authorization и `tests/ui-refresh-coordinator.test.mjs` для scoped city refresh.
...
- **Окна:** `openTrader()`, `openTraderV2()`, `openTraderSheet()` обязаны делегировать единственному `scripts/ui/trader-app-v2.js`.
...
- **City view flow:** `CityEconomyApp._prepareContext()` повторно применяет `resolveCityViewMode({ isGM, requestedMode })`: player всегда получает только `getPublicCitySnapshot(cityId)`, GM по умолчанию сохраняет analytics/admin context и переключает ту же instance в public mode. Public tabs whitelisted как `city|market|traders`; `open-trader` делегирует только `moduleApi.openTrader(cityId, traderKey)` без travel authorization.
...
- **Тесты:** `tests/ui-refresh-coordinator.test.mjs` (включая scoped city selection), `tests/main-composition-root.test.mjs`, `tests/city-public-ui.test.mjs`, `background-refresh-focus.test.mjs`, `style-theme.test.mjs` и профильные `*-app-context.test.mjs`/`*-ui.test.mjs`.
