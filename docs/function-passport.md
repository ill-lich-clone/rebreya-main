...
- **Travel map lifecycle:** `new TravelMapService({ gameProvider, canvasProvider })` и `syncGroupToken({ groupActor, position })` не двигают token document, пока целевая сцена не является отрисованной canvas scene или `token.object === null`; возвращается `{ synced: false, deferred: true }`. `registerTravelMapHooks(moduleApi)` только на active GM повторяет проекцию через `syncTravelMapToken()` после `canvasReady`. `setTravelRoute(payload)` возвращает snapshot без глобального `refreshOpenApps()`, поэтому открытый travel UI не перерисовывается и не закрывается.
