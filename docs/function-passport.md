...
- **Typed commands:** `storage.open`, `storage.journal.read`, `storage.claim-row`, `storage.claim-coins`, `storage.deposit`, `storage.drop-item-to-scene`, `storage.restore-portable`, `storage.token-to-character`.
...
- **Journal read contract:** `createStorageJournalHtmlParser(documentProvider)` builds the injected Foundry-browser adapter by assigning enriched HTML to an inert `<template>` and returning its `content`; focused tests exercise this production adapter through an independent dependency-free semantic template contract. `StorageJournalReader.read(journalUuid)` live-resolves only `JournalEntry`, sorts its pages by numeric `sort` then ID, enriches text through Foundry 13 `CONFIG.ux.TextEditor.implementation.enrichHTML` with `secrets:false` and documents/links/embeds/rolls/custom disabled, parses the enriched fragment through that adapter, rejects `section.secret:not(.revealed)`, and returns only plain `{ name, pages }` data with whitelisted page fields plus derived `html`. Resolution/enrichment/parsing/deletion failures collapse to one unavailable error; Journal/page UUIDs, ownership, flags, sheet/embedded document data and raw `text.content` never enter the snapshot, and the reader never updates the Journal.
