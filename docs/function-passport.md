...
- **Космология:** расчёты Mechanus — `scripts/cosmology/mechanus-rolls.js`; явный нулевой dice count сохраняется как `0`, а advantage/disadvantage d20 всегда заменяет системный keep-результат первым брошенным кубом и отдельным сериализуемым `NumericTerm` `+2`/`-2`, чтобы dnd5e/MIDI total оставался согласован с terms после пересчёта и записи в ChatMessage; внешние методы `getCosmologyState()`, `isMechanusEnabled()`, `setMechanusEnabled(enabled)`, `openCosmologyApp()`; typed command `cosmology.setMechanus`; UI — `scripts/ui/cosmology-app.js`.
