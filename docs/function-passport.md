...
- **Космология:** расчёты Mechanus — `scripts/cosmology/mechanus-rolls.js`; явный нулевой dice count сохраняется как `0`, а advantage/disadvantage d20 всегда заменяет системный keep-результат первым брошенным кубом и отдельным сериализуемым `NumericTerm` `+2`/`-2`. `applyMechanusAveragesToRoll(roll, { enabled })` валидирует добавленный term, пересобирает `_formula` и `_total` из `terms`; `registerMechanusRollHooks(moduleApi)` дополнительно регистрирует единственный `preCreateChatMessage`, который повторяет нормализацию на финальной границе сохранения dnd5e/MIDI и обновляет простое числовое `content`. Внешние методы `getCosmologyState()`, `isMechanusEnabled()`, `setMechanusEnabled(enabled)`, `openCosmologyApp()`; typed command `cosmology.setMechanus`; UI — `scripts/ui/cosmology-app.js`.
