...
export class TradeTransactionService {
  ...
  async #resumeApplication(transactionId) {
    try {
      ...
      if (transaction.phase === "item-applied") {
        const receipts = await this.#readReceipts(transaction);
        if (!receipts.itemApplied) {
          return this.#failInconsistentRecovery(transaction, "item-applied");
        }
      ...
      else if (transaction.phase === "currency-applied") {
        const receipts = await this.#readReceipts(transaction);
  ...
  async #compensateFailure(transactionId, failure) {
    ...
    try {
      ...
      await this.#repository.mutateTransaction(transactionId, (row) => {
    ...
    try {
      ...
      if (incrementAttempt) {
        await this.#checkpoint(transactionId, (row) => {
          ...
          row.compensation.attempts = Math.max(0, Number(row.compensation.attempts) || 0) + 1;
        });
        transaction = this.#requireTransaction(transactionId);
      }
      ...
      const phase = transaction?.phase === "currency-compensated"
        ? "compensating-item"
        : (transaction?.phase === "item-compensated"
          ? "releasing-stock"
          : (transaction?.phase === "stock-released" ? "finishing-compensation" : "compensating-currency"));
      return this.#markReconciliationRequired(transactionId, originalError, error, phase);
    }
  }
  ...
  async #markReconciliationRequired(transactionId, originalError, compensationError, phase) {
    try {
      await this.#repository.mutateTransaction(transactionId, (row) => {
        ...
        row.error ??= sanitizeError(originalError, row.error?.phase ?? row.phase);
  ...
  async #checkpoint(transactionId, mutator) {
    return this.#repository.mutateTransaction(transactionId, (row, state) => {
      mutator(row, state);
      row.updatedAt = this.#timestamp();
      return clone(row);
    });
