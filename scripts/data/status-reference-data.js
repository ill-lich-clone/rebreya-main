...
function freezeDefinition(statusId, definition = {}) {
  const frozen = {
    ...definition,
    statusId,
  ...
  for (const key of ["paragraphs", "bullets", "seeAlso"]) {
    if (Array.isArray(frozen[key])) {
      frozen[key] = Object.freeze([...frozen[key]]);
    }
  }
  if (frozen.source && typeof frozen.source === "object") {
    frozen.source = Object.freeze({ ...frozen.source });
  }
...
const definitions = {
  ...
  bloodied: {
    ...
    aliases: ["окровавленного", "окровавленным"],
    subtitle: "Состояние Rebreya",
    source: { file: "Глоссарий.txt", label: "Lich" },
    paragraphs: [
...
export const STATUS_REFERENCE_DATA = Object.freeze(Object.fromEntries(
  Object.entries(definitions).map(([statusId, definition]) => [
    statusId,
    freezeDefinition(statusId, definition)
  ])
));

export function getStatusReferenceDefinition(statusId) {
  return STATUS_REFERENCE_DATA[String(statusId ?? "").trim()] ?? null;
}

export function renderStatusReferenceDescription({
  paragraphs = [],
  bullets = [],
  valueHint = "",
  seeAlso = []
