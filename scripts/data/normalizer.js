...
import { canonicalCatalogId } from "../shared/canonical-catalog-id.js";
...
function slugifyBase(value) {
  return canonicalCatalogId(value);
}
...
function normalizeCities(rawCities, goodAliasMap, regionAliasMap) {
  ...
  const cities = rawItems.map((record, index) => {
    ...
    return {
      ...
      image: cleanString(getValue(record, CITY_FIELD_ALIASES.image)),
...
function normalizeGear(rawGear, materialAliasMap, materials) {
  ...
  const gear = rawItems.map((record, index) => {
    ...
    return {
      ...
      multipleAppearance: cleanString(getValue(record, GEAR_FIELD_ALIASES.multipleAppearance, "1")) || "1",
