...
import { resolveLootgenContainerProfile } from "./lootgen-container-rules.js?v=1.4.265";
...
export function createLootgenCatalogReader({model={},gearIndex=[],magicDocuments=[],manifest=[]}={}) {
  ...
  function physicalSource(component){
    return component?.sourceType==="gear"?gearDocuments.get(id(component.sourceId))
      :component?.sourceType==="magicItem"?magic.get(id(component.sourceId)):null;
  }
  return Object.freeze({
    readContainerValueNodes,
    resolveContainerProfile(component){return resolveLootgenContainerProfile(physicalSource(component));},
    readPhysicalItem(component){
      const source=physicalSource(component);if(!source)return null;
      const {quantity,weight,volume,capacity,properties}=source.system??{};
      return structuredClone({type:source.type,system:{quantity,weight,volume,capacity,properties}});
    },
