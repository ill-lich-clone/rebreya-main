...
export function chooseLootgenUpgradeVariant({ host, remainingValue, form, catalogReader, manifest, random, createInstanceKey, attemptBudget = { remaining:2000 } }) {
  ...
  const prospectiveHost={...descriptor,quantity:1};
  const pool=(manifest??[]).filter(entry=>["simple-implemented","existing-curse"].includes(entry.decision)
    ...
    .filter(entry=>{
      try {
        validateUpgradeInstallation(prospectiveHost,[],{profile:entry.profile,availability:entry.decision});
        const choices=getUpgradeChoiceOptions(entry.productId);
        validateUpgradeChoices(entry.productId,choices.length?{damageType:choices[0]}:{});
        return true;
      } catch(error) { diagnostics.push({sourceId:entry.productId,reason:error.message});return false; }
    });
  ...
  while(pool.length && preview.upgrades.length < Math.min(3,form.maxUpgradesPerItem,descriptor.capacity) && attemptBudget.remaining-- > 0) {
    ...
    try {
      const {slotIndex}=validateUpgradeInstallation(prospectiveHost,preview.upgrades,{profile:entry.profile,availability:entry.decision});
