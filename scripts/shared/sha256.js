// Synchronous SHA-256 for deterministic, bounded identity checks in synchronous inventory projections.
...
export function sha256Hex(value) {
  ...
  const padded=new Uint8Array(length);padded.set(bytes);padded[bytes.length]=0x80;
  ...
  for(let offset=0;offset<length;offset+=64){
    ...
    const round=[a,b,c,d,e,f,g,h];for(let i=0;i<8;i++)state[i]=(state[i]+round[i])>>>0;
  }
