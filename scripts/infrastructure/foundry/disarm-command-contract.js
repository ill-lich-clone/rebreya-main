...
export const DISARM_ACTIONS = ["preview", "start", "resolve-save", "set-baseline", "resume", "cancel"];
...
export function authorizeDisarmSender(action, { sender, game = globalThis.game } = {}) {
  return Boolean(sender?.id && game?.users?.get(sender.id) === sender && (action !== "set-baseline" || sender.isGM === true));
}
