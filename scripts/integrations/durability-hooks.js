...
export function registerDurabilityHooks(moduleApi, {
  ...
  Hooks.on(ITEM_PILES_PRE_CREATE_HOOK, (tokenData, items) => {
    if (!itemPilesApi(foundryGame)) {
      return true;
    }
    const item = singleDurabilityItem(items);
    if (item) {
      projectDurabilityOntoTokenData(tokenData, item);
    }
    return true;
  });
  ...
  Hooks.on("dnd5e.preApplyDamage", (actor, amount, _updates, options = {}) => {
    const context = pileDamageContext(moduleApi, actor, amount, foundryGame);
    if (!context) {
      return true;
    }
    void reportPileTask(
  ...
  Hooks.on("preUpdateActor", (actor, changed, options = {}) => {
    if (options?.[MODULE_ID]?.[PILE_PROJECTION_OPTION] === true) {
      return true;
    }
    const currentHp = Number(getPropertyValue(actor, "system.attributes.hp.value"));
    const nextHp = Number(getPropertyValue(changed, "system.attributes.hp.value"));
    if (!Number.isFinite(currentHp) || !Number.isFinite(nextHp)) {
      return true;
    }
    const pileContext = pileDurabilityContext(moduleApi, actor, foundryGame);
    if (!pileContext) {
      return true;
    }
    setChangedProperty(changed, "system.attributes.hp.value", currentHp);
    if (nextHp >= currentHp) {
      return true;
    }
  ...
  Hooks.on("deleteItem", (item) => reportPileTask(
    cleanupDestroyedPileItem(item, { game: foundryGame, isActiveGm }),
    "Failed to remove an empty destroyed Item Pile."
  ));
