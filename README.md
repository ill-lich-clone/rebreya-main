...
При `initialize()` и `reloadData()` active GM автоматически обновляет managed-компендиум магических предметов и все распознанные копии на character Actor: надетые, настроенные и лежащие в инвентаре. Полный аудит 655 stable catalog rows хранится в [`docs/magic-item-automation-audit.md`](docs/magic-item-automation-audit.md): простые native `+1/+2/+3`, плоские эффекты, подтверждённые сопротивления/скорости/зрение и explicit spell/utility/save activities публикуются как `full` либо `partial`, а условные и неоднозначные правила получают явный `manual` reason. Несколько activities одного предмета расходуют общий `system.uses`, когда описание задаёт общий пул; независимые `1/dawn` свойства используют отдельные activity/item resources.
