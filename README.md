...
- `claimStorageAll(tokenUuid, destination, mutationId, request)` выполняет `Залутать всё` одной typed-командой `storage.claim-all`. `destination` — `self` либо `party`; для группы API фиксирует exact `request.target:{groupActorId,folderId}`. Операция последовательно забирает полное количество обычных строк и переносимых контейнеров, затем все монеты, пропускает Journal-ссылки и использует стабильные дочерние mutation ID, поэтому retry продолжает незавершённую выдачу без дублей.
