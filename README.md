...
- Внизу окна Lootgen мастер сохраняет текущие настройки кнопкой `Сохранить шаблон`. Каждый шаблон — обычный world Item типа `rebreya-main.lootgen-template` в папке `Шаблоны Lootgen`; Item можно открыть его собственным листом, переименовать и перетащить из sidebar или compendium. Старый скрытый setting `lootgenTemplates` остаётся только входом одноразовой возобновляемой миграции и не является рабочим каталогом.
