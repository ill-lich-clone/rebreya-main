...
- `Общий компендиум снаряжения V0.1` — единственный source четырёх Coin Items; штатный импорт публикует их как managed treasure-документы в `world.rebreya-gear/Сокровища` и добавляет стабильный `flags.rebreya-main.storageCoinTemplate = {version:1, denomination}`. Отдельные world Item-шаблоны и папка `МОНЕТЫ` не создаются; active GM удаляет только старые документы с legacy managed-флагом и их опустевшую legacy-папку.
