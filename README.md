...
- Вне активного Notes layer перетаскивание exact `JournalEntry` вызывает `dropStorageJournalToScene()` и GM-only typed command `storage.journal.drop-to-scene`; `JournalEntryPage` не перехватывается. Active GM повторно разрешает исходный Journal через canonical deposit source, принимает только reference-only строку quantity `1`, не изменяет Journal и передаёт detached row существующему ground-pile owner. Одна такая корневая строка показывает исходное имя и отдельную иконку заметки, две и более — `Куча заметок` с отдельной pile-иконкой; обычные Item имеют priority над Journal и монетами, монеты — над Journal-only fallback, а preserved empty coin pile остаётся монетной. Mutation ID связан с Journal, сценой, координатами и sender; retry не создаёт вторую строку, а ошибка scene transfer вызывает безопасный copy rollback и при двойной ошибке сохраняет обе причины.
