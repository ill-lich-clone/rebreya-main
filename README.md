...
- API мастера `prepareLootgenGeneratedResult(form, { operationId? })` подготавливает улучшенный лут при `enableUpgrades:true` через active GM. Возвращает `{messageId,lootId,state}` закрытого черновика; повтор того же operationId/form возвращает сохранённый результат. Стабильный operationId хранится в операции, сетевой повтор использует новый request ID. В настройках Lootgen доступны включение усовершенствований, шанс, максимум на предмет, типы и ранги. Пустой фильтр означает все доступные варианты. Окно сохраняет полный состав и стоимость; выдача и публикация используют подготовленную запись.
