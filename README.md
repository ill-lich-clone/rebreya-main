...
- GM-настройки root/nested хранилища показывают число активных цепочек и открывают отдельный широкий редактор триггеров. События: до/после открытия, после получения и опустошение; встроенные шаги включают условия, dnd5e проверки/спасброски, расход предмета, урон, сообщения, переменные, ветвление, allow/deny/finish и Macro. Готовые шаблоны создают «Замок» и ловушку со спасброском Ловкости; cooldown/перезарядки нет. Definition read/save/reset проходят только через GM-only `storage.triggers.read|save|reset`, save использует revision, reset очищает срабатывания, но сохраняет definitions и variables.
