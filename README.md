...
- После обычного чтения storage Journal viewer предлагает действие `Записать`. `recordStorageJournal(tokenUuid,rowId,mutationId,request)` повторяет live access-проверки на active GM и создаёт у конкретного читающего character Actor один dnd5e `loot` Item с точным `flags.rebreya-main.journalRecord = {version:1,sourceUuid,documentName}`. Повтор для той же Journal-ссылки у того же Actor возвращает существующий Item (`created:false`), а после удаления Item запись можно создать заново. Прямой клик по названию такого Item в штатном character inventory вызывает `readJournalRecord(itemUuid)`: active GM проверяет Item, parent Actor и OWNER-доступ, повторно читает исходную JournalEntry/JournalEntryPage и открывает тот же viewer без action-кнопок. Обычное открытие Journal и остальных Item не перехватывается.
