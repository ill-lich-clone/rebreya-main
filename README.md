...
При `initialize()` и `reloadData()` active GM автоматически обновляет managed-компендиум магических предметов и все распознанные копии на character Actor: надетые, настроенные и лежащие в инвентаре. Пишутся только реальные изменения по authoritative Foundry document diff. Ручной effect игрока имеет приоритет над совпадающими change keys модуля, поэтому двойной бонус не создаётся; непересекающаяся managed-механика сохраняется. Эквивалентные старые spell activities переводятся на stable managed activities с сохранением потраченных зарядов. `syncOwnedMagicItems({ dryRun, reportToConsole })` остаётся диагностическим API, а `syncEquippedMagicItems(options)` — совместимым alias; для штатного обновления консоль не нужна. `Особый Кинжал телепортации`, `Зелье заживления ран` и `Зелье лечения 1-го уровня` намеренно пропускаются.
