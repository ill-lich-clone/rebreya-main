...
- `readStorageJournal(tokenUuid, rowId, request)` читает только Journal-ссылку из открытого, доступного хранилища. Active GM заново проверяет сцену, видимость, дистанцию и владение выбранным персонажем, берёт Journal UUID только из authoritative строки по `request.path`, читает актуальные страницы без изменения документа или ownership и возвращает UUID-free read-only snapshot без нераскрытых secret-блоков. После первого успешного чтения строка получает общую token/container-scoped отметку, корневая наземная presentation обновляется и запись показывается всем как `<исходное имя> (прочитано)`; одновременно создаётся одно публичное сообщение только с очищенными именами читателя и записи. Повторное чтение не обновляет кучу и не публикует сообщение, а ошибка чата не откатывает уже сохранённую отметку. Исходный Journal и его имя не изменяются. Sidebar visibility и ownership Journal не дают и не отменяют доступ; удалённая, забранная или небезопасная запись отвечает общей ошибкой недоступности.
