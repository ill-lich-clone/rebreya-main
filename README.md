...
- API мастера `prepareLootgenGeneratedResult(form, { operationId? })` подготавливает составной лут при `enableUpgrades:true` или `enableFilledContainers:true` через active GM. Возвращает `{messageId,lootId,state}` закрытого черновика; socket передаёт только ссылки, полный state читается из сохранённого ChatMessage после репликации. Заполненный контейнер содержит всё подготовленное дерево и общую стоимость; его дочерние строки нельзя забирать отдельно из Chat. Повтор того же operationId/form возвращает сохранённый результат. Стабильный operationId хранится в операции, сетевой повтор использует новый request ID. В настройках Lootgen доступны включение усовершенствований, шанс, максимум на предмет, типы и ранги. Пустой фильтр означает все доступные варианты. Окно сохраняет полный состав и стоимость; выдача и публикация используют подготовленную запись.
