...
- API мастера `prepareLootgenGeneratedResult(form, { operationId? })` подготавливает улучшенный лут при `enableUpgrades:true` через active GM. Возвращает `{messageId,lootId,state}` закрытого черновика; повтор того же operationId/form возвращает сохранённый результат. Предметы пока не выдаются. Публикация/выдача этого нового черновика и настройки улучшений в окне добавляются следующим шагом R8.
