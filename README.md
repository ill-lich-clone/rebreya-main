...
- Manifest версии `1.4.162` загружает только тонкий `scripts/main-1.4.162.js`, который импортирует единственный composition root `scripts/main.js` с cache-bust `1.4.162-gm-journal-read`; изменённый storage command module использует тот же release key.
