...
- Разбор разрешён только Item с актуальной папкой. Полученный материал сначала проходит единственный ingress planner: совпавшее folder/skip правило применяется как обычно, а при отсутствии совпадения fallback остаётся исходная папка. Stale или невозможная цель завершается без переноса в корень.
