...
- Дверь можно включить как цель тех же триггеров через `Ctrl+ПКМ` мастера. В редакторе доступны только `До открытия` и `После открытия`, включая шаблоны замка и ловушки. Обычный ПКМ мастера и открытая дверь сохраняют native Foundry-поведение. ЛКМ по включённой закрытой/запертой двери показывает одну кнопку `Открыть`; игрок и мастер должны выбрать принадлежащего им персонажа на той же сцене не дальше 10 футов. Секретная дверь не раскрывается игроку. Проверки и изменение `Wall.ds` выполняет active GM через `door.open`, а definition read/save/reset — через GM-only `door.triggers.read|save|reset`.
