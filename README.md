...
- Manifest версии `1.4.164` загружает только тонкий `scripts/main-1.4.164.js`, который импортирует единственный composition root `scripts/main.js` с cache-bust `1.4.164-storage-config-no-open`; изменённые storage trigger/command/token-hook модули используют release key `1.4.164-storage-key-feedback`.
