...
- API мастера `prepareLootgenGeneratedResult(form, { operationId? })` подготавливает улучшенный лут при `enableUpgrades:true` через active GM. Возвращает `{messageId,lootId,state}` закрытого черновика; повтор того же operationId/form возвращает сохранённый результат. Стабильный operationId хранится в операции, сетевой повтор использует новый request ID. Настройки улучшений в окне ещё разрабатываются (R8); подготовка и публикация доступны через API.
