...
- После обычного чтения storage Journal viewer предлагает действие `Записать`. `recordStorageJournal(tokenUuid,rowId,mutationId,request)` повторяет live access-проверки на active GM и создаёт один dnd5e `loot` Item с точным `flags.rebreya-main.journalRecord = {version:1,sourceUuid,documentName}`: игроку — у конкретного читающего character Actor, мастеру — в захваченной перед запросом текущей активной группе. Повтор для той же Journal-ссылки у того же целевого Actor возвращает существующий Item (`created:false`), а после удаления Item запись можно создать заново. Прямой клик по названию такого Item в штатном inventory вызывает `readJournalRecord(itemUuid)`: active GM проверяет Item, parent Actor и доступ, повторно читает исходную JournalEntry/JournalEntryPage и открывает тот же viewer без action-кнопок. Обычное открытие Journal и остальных Item не перехватывается.
