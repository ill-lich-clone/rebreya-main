...
При `initialize()` и `reloadData()` active GM автоматически обновляет managed-компендиум магических предметов и все распознанные копии на character Actor: надетые, настроенные и лежащие в инвентаре. Полный rarity-aware аудит 655 stable catalog rows хранится в [`docs/magic-item-automation-audit.md`](docs/magic-item-automation-audit.md): 210 строк имеют статус `full`, 170 — `partial`, 275 — `manual`. Простые native `+1/+2/+3`, плоские эффекты, подтверждённые сопротивления/скорости/зрение и explicit spell/utility/save activities публикуются как `full` либо `partial`; для `partial` manifest точно перечисляет оставшуюся ручную механику, а условные и неоднозначные правила получают явный `manual` reason. Несколько activities одного предмета расходуют общий `system.uses`, когда описание задаёт общий пул; независимые свойства используют отдельные activity resources, а невосстанавливаемые заряды получают пустой recovery-массив. Бонусы, ограниченные заклинаниями конкретного класса, не превращаются в глобальные Active Effects.
