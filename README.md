...
- При первом явном открытии содержимое генерируется один раз, а active GM отправляет один transient positional-звук без создания AmbientSound на сцене. Игрок может передать каждую строку или монеты себе либо в групповой инвентарь. Для игрока active-GM повторно проверяет видимость, владение выбранным персонажем и дистанцию не более 5 футов.
- После успешной выдачи строки или монет себе/в групповой инвентарь authoritative active GM публикует видимое всем сообщение с именем инициатора, количеством/номиналами и назначением. Повтор той же mutation ID, отказ, validation error и запрещённая Journal-ссылка не публикуют сообщение; UUID и внутренние flags в текст не попадают.
- В Storage UI блок `Залутать всё` предлагает прежние назначения `Себе` и `В группу` и скрывается, если остались только Journal-ссылки. Он вызывает `claimStorageAll(tokenUuid, destination, mutationId, request)` одной typed-командой `storage.claim-all`. Для группы API фиксирует exact `request.target:{groupActorId,folderId}`. Операция последовательно забирает полное количество обычных строк и переносимых контейнеров, затем все монеты, пропускает Journal-ссылки и использует стабильные дочерние mutation ID, поэтому retry продолжает незавершённую выдачу и повторяет незавершённый refresh без дублей. Пока массовая выдача не завершена, её mutation ID durably связан в token/container state с отправителем, персонажем, путём и назначением исходного запроса; хранилище сохраняет до 100 pending bindings и последние 100 terminal receipts. Cached/in-flight retry заново проходит live access и target-проверки. Исключение — уже удалённая финальная куча: её compact success можно повторно получить только по тому же fingerprint, потому что source UUID больше не разрешается. Мутации корня и всех вложенных контейнеров одного Scene Token сериализуются общей очередью, поэтому параллельные действия в соседних сумках не перезаписывают друг друга.
