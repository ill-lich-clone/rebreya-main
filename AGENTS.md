...
Сам оценивай, выгоднее ли продолжать текущий чат или передать следующий этап в новый. Не жди отдельного вопроса пользователя: если переход уменьшит лишний контекст или повысит надёжность, явно предложи новый чат и укажи рекомендуемую модель.

- `Luna` предлагай для полностью описанной механической работы с низким риском: поиск и классификация, однотипные правки, обновление документации по готовому diff, запуск проверок.
- `Terra` используй как основной выбор для реализации готовой спецификации, обычных bugfix, focused-тестов, UI и изменений одного владельца поведения.
- `Sol` предлагай для архитектурных решений, неясных или повторно не исправленных дефектов, миграций и изменений, затрагивающих несколько слоёв, права GM, sockets, recovery, идемпотентность или сохранность world-state. Также предлагай `Sol` для финального ревью рискованного крупного изменения.
