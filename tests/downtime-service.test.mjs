...
test("grantWeeks uses the explicit group calendar instead of the active group calendar", async () => {
  const actor = createActor({ id: "actor-a", name: "Hero A" });
  const harness = createHarness({
    members: [actor],
    calendarIsoDate: "2026-07-16",
  ...
  try {
    const resolveForGroup = harness.groupContextService.resolveForGroup.bind(harness.groupContextService);
    harness.groupContextService.resolveForGroup = (groupId) => ({
      ...resolveForGroup(groupId),
      groupState: {
        ...resolveForGroup(groupId).groupState,
        calendar: { version: 1, isoDate: "2026-08-06", timeOfDaySeconds: 0 }
      }
    });

    await harness.service.grantWeeks({
      groupId: harness.groupActor.id,
      actorIds: [actor.id],
      weeks: 1
    });

    assert.equal(getDowntimeState(harness).grants[0].anchorMonday, "2026-08-10");
  }
  finally {
    harness.restore();
  }
