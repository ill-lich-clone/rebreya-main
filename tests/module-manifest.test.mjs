...
test("module manifest loads an unpinned canonical entrypoint for page-refresh updates", async () => {
  ...
  assert.deepEqual(manifest.esmodules, ["scripts/main-1.4.209.js"]);
...
test("current entrypoint cache-busts the changed craft durability and transfer graph", async () => {
  ...
  assert.equal(
    groundPileServiceSource.includes("storage-pile-presentation.js?v=1.4.209-top-down-item-presentation"),
    true,
    "ground-pile presentation changes need their own browser module cache key"
  );
  assert.equal(
    canonicalSource.includes("storage-ground-pile-service.js?v=1.4.209-top-down-item-presentation"),
    true,
