...
test("module manifest loads an unpinned canonical entrypoint for page-refresh updates", async () => {
  ...
  assert.equal(manifest.version, "1.4.211");
  assert.deepEqual(manifest.esmodules, ["scripts/main-1.4.211.js"]);
...
test("current entrypoint cache-busts the changed craft durability and transfer graph", async () => {
  ...
  for (const importPath of [
    ...
    "data/inventory-service.js?v=1.4.179-dismantle-minimum-quantity",
    ...
    "data/storage-ground-pile-service.js?v=1.4.211-furniture-footprints",
  ...
  assert.equal(
    groundPileServiceSource.includes("storage-pile-presentation.js?v=1.4.211-furniture-footprints"),
    true,
    "ground-pile presentation changes need their own browser module cache key"
  );
  assert.equal(
    canonicalSource.includes("storage-ground-pile-service.js?v=1.4.211-furniture-footprints"),
    true,
