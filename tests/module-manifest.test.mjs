...
test("module manifest loads an unpinned canonical entrypoint for page-refresh updates", async () => {
  ...
  assert.equal(manifest.version, "1.4.300");
  assert.deepEqual(manifest.esmodules, ["scripts/main-1.4.300.js"]);
...
test("Lootgen window import uses the current release cache version", async () => {
  const manifest = JSON.parse(await readFile(new URL("../module.json", import.meta.url), "utf8"));
...
test("production registers the hidden GiantTribe advancement before race compendiums are materialized", async () => {
  ...
  assert.match(
    entrypointSource,
    /dnd5e-sheet-extensions\.js\?v=1\.4\.300/u
  );
  assert.match(
    entrypointSource,
...
test("current entrypoint cache-busts the changed craft durability and transfer graph", async () => {
  ...
  assert.match(
    canonicalSource,
    /integrations\/dnd5e-sheet-extensions\.js\?v=1\.4\.300/u
  );
...
test("module stylesheet cache bust loads the storage deposit interaction styles", async () => {
  ...
  assert.match(entrypointSource, /const MODULE_STYLE_VERSION = "1\.4\.300";/u);
