...
test("module manifest loads an unpinned canonical entrypoint for page-refresh updates", async () => {
  ...
  assert.equal(manifest.version, "1.4.229");
  assert.deepEqual(manifest.esmodules, ["scripts/main-1.4.229.js"]);
...
test("canonical entrypoint cache-busts the player-list inventory token launcher", async () => {
  ...
  assert.match(
    entrypointSource,
    /hooks\.js\?v=1\.4\.172-panel-owner-runtime/u
  );
  assert.match(
    entrypointSource,
...
test("production registers the hidden GiantTribe advancement before race compendiums are materialized", async () => {
  ...
  assert.match(
    entrypointSource,
    /dnd5e-sheet-extensions\.js\?v=1\.4\.221-journal-readonly-dialog/u
  );
  assert.match(
    entrypointSource,
...
test("current entrypoint cache-busts the changed craft durability and transfer graph", async () => {
  ...
  assert.match(
    canonicalSource,
    /integrations\/dnd5e-sheet-extensions\.js\?v=1\.4\.221-journal-readonly-dialog/u
  );

  for (const importPath of [
    "data/trader-service.js?v=1.4.109-lazy-trader-restock",
    "data/downtime-service.js?v=1.4.96-craft-calendar",
    "data/inventory-service.js?v=1.4.226-inventory-transfer",
  ...
  for (const [source, importPath] of [
    ...
    [lootgenAppSource, "data/lootgen-generator.js?v=1.4.225-physical-coins"],
  ...
  assert.equal(
    groundPileServiceSource.includes("storage-pile-presentation.js?v=1.4.227-coin-sprites"),
    true,
    "ground-pile presentation changes need their own browser module cache key"
  );
  assert.equal(
    canonicalSource.includes("storage-ground-pile-service.js?v=1.4.227-coin-sprites"),
    true,
...
test("automatic owned magic item sync cache-busts its live service graph", async () => {
  ...
  assert.match(
    compendiumSource,
    /magic-item-embedded-sync\.js\?v=1\.4\.192-unsupported-item-activity-repair/u
  );
});
