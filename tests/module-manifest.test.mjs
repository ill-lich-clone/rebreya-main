...
test("module manifest loads the stable canonical entrypoint", async () => {
  ...
  assert.equal(manifest.version, "1.4.139");
  assert.deepEqual(manifest.esmodules, ["scripts/main-1.4.139.js"]);
...
test("combat automation imports preserve their released cache busts", async () => {
  ...
  assert.match(
    entrypointSource,
    /mechanus-rolls\.js\?v=1\.4\.138-mechanus-midi-chat-repair/u,
  );
  assert.match(
    entrypointSource,
