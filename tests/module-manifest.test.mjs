...
test("module manifest loads an unpinned canonical entrypoint for page-refresh updates", async () => {
  ...
  assert.equal(manifest.version, "1.4.247");
  assert.deepEqual(manifest.esmodules, ["scripts/main-1.4.247.js"]);
...
test("production registers the hidden GiantTribe advancement before race compendiums are materialized", async () => {
  ...
  assert.match(
    entrypointSource,
    /dnd5e-sheet-extensions\.js\?v=1\.4\.247-hero-overlays/u
  );
  assert.match(
    entrypointSource,
...
test("current entrypoint cache-busts the changed craft durability and transfer graph", async () => {
  ...
  assert.match(
    canonicalSource,
    /integrations\/dnd5e-sheet-extensions\.js\?v=1\.4\.247-hero-overlays/u
  );
