...
test("module manifest loads the stable canonical entrypoint", async () => {
  ...
  assert.equal(manifest.version, "1.4.186");
  assert.deepEqual(manifest.esmodules, ["scripts/main-1.4.186.js"]);
...
test("automatic owned magic item sync cache-busts its live service graph", async () => {
  ...
  assert.match(
    entrypointSource,
    /magic-items-compendium\.js\?v=1\.4\.186-manual-automation-migration/u
  );
  assert.match(
    compendiumSource,
    /magic-item-embedded-sync\.js\?v=1\.4\.186-manual-automation-migration/u
  );
});
