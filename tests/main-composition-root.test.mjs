...
import { TransportCompendiumService } from "../scripts/data/transport-compendium.js?v=1.4.323";
...
test("release 1.4.323 reuses the catalog icon cache graph", async () => {
  ...
  assert.equal(manifest.version, "1.4.323");
  assert.deepEqual(manifest.esmodules, ["scripts/main-1.4.323.js"]);
  assert.doesNotMatch(manifestSource, /main-1\.4\.319\.js/u);
  assert.match(mainSource, /data\/inventory-service\.js\?v=1\.4\.323/u);
  assert.match(mainSource, /data\/storage-command-service\.js\?v=1\.4\.322/u);
  assert.match(mainSource, /integrations\/inventory-sync\.js\?v=1\.4\.323/u);
  assert.match(syncSource, /data\/inventory-service\.js\?v=1\.4\.323/u);
  assert.match(appSource, /integrations\/inventory-sync\.js\?v=1\.4\.323/u);
  assert.match(serviceSource, /gear-icon-resolver\.js\?v=1\.4\.323/u);
  assert.equal(mainSource.match(/ui\/inventory-app\.js\?v=1\.4\.323/gu)?.length ?? 0, 3);
  ...
  for (const name of iconServiceNames) {
    assert.match(mainSource, new RegExp(`data/${name}\\.js\\?v=1\\.4\\.323`, "u"));
  }
  assert.match(mainSource, /data\/compendium-utils\.js\?v=1\.4\.323/u);
  assert.match(mainSource, /data\/trader-service\.js\?v=1\.4\.323/u);
  ...
  for (const source of cacheImporterSources) {
    assert.match(source, /compendium-utils\.js\?v=1\.4\.323/u);
  }
...
test("composition root synchronizes the managed transport Actor compendium", async () => {
  ...
  assert.match(source, /import\s+\{\s*TransportCompendiumService\s*\}\s+from\s+"\.\/data\/transport-compendium\.js\?v=1\.4\.323";/u);
...
test("composition root owns one inventory ingress graph and one batch dispatch helper", async () => {
  ...
  assert.match(
    source,
    /\.\/data\/inventory-service\.js\?v=1\.4\.323/u,
